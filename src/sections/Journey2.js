...
export default function JourneyComponent({ journeyData, gsap, ScrollTrigger }) {
	useEffect(() => {
		//
		const journeyAnimTimeline = gsap.timeline({});

		/** animation journey */
		function journeyAnimation() {
			journeyAnimTimeline.to(".ContentBx", 2, { x: "0", opacity: "1" }, "first");
			// 	.to(
			// 		`.${styles.HandImg}`,
			// 		2,
			...
			ScrollTrigger.create({
				animation: journeyAnimTimeline,
				trigger: `${styles.Journey_box_wrapper}`,
				start: "200px 100px",
				end: "bottom bottom",
				scrub: 1,
				pin: true,
