		...
		<section className={`${styles.AvantGarde} ptb_80`}>
			<div className="container">
				<div className={styles.titleBox}>
					<h2 className="text_xxxl color_white">Avant-Garde 2022 Results</h2>
				</div>
					...
					{avantGarde.map((item) => (
						<div className={styles.itemContent} key={item.title}>
							<div className={`${styles.title} text_center`}>
								<h4 className="text_lg font_primary color_white f_w_m">{item.title}</h4>
							</div>
							<div className={`${styles.info} text_center`}>
								...
								<p className="text_xs color_primary opacity_8">{item.name}</p>
							</div>
						</div>
					))}
				</div>
